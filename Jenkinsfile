pipeline {
    ...
    stages {
            ...
            steps {
                ...
                script {
                    if ( env.BRANCH_NAME == 'master' ) {
                        containerVersion = getVersionFromContainer("liatrio/ldop-ldap-ltb:${env.BRANCH_NAME}")
                        failIfVersionExists("liatrio","ldop-ldap-ltb",containerVersion)
                        sh "docker build -t liatrio/ldop-ldap-ltb:${containerVersion} ."
                    }
                }
            }
            post {
        ...
        stage('Push to dockerhub') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-ldap-ltb:${env.BRANCH_NAME} liatrio/ldop-ldap-ltb:latest"
                sh "docker push liatrio/ldop-ldap-ltb:latest"
                script {
                    if ( env.BRANCH_NAME == 'master' )
                        sh "docker push liatrio/ldop-ldap-ltb:${containerVersion}"
                }
            }
            post {
                success { script { STATUS = "SUCCESS" } }
